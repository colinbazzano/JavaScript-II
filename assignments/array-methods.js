...
// you can use 'const firstNamesAllCaps = runner.map' and so on
...
// Problem 1
//I want to find out who is in the 'big baller' status, please find who donated over $250.
let bigBallers = [];
  runners.filter(function(item) {
    if(item.donation > 250) {
    bigBallers.push(item);
    }
  });
console.log(bigBallers);

// Problem 2
//We want to send thank yous out to all the runners emails, with a list to remind me to thank them. Please make an array of people's emails with "Send a thank you to `email` please!"
let emails = [];
runners.forEach(function(item){
  emails.push(`Send a thank you to ${item.email} please!`);
})
console.log(emails);
// Problem 3

//the event director has sobered up and is embarrased with his actions. Please make the first names lower case so they aren't being yelled.
let firstNamesLowerCase = [];
runners.map(function(item) {
  firstNamesLowerCase.push(item.first_name.toLowerCase());
});
console.log(firstNamesLowerCase);